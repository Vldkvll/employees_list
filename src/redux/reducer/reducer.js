...
export const apiReducer = ( state = INITIAL_STATE, action) => {
    switch (action.type) {
        case API_ACTIONS.FETCH_START:
            const innerStart = camelCase(action.type.replace(API_ACTIONS.FETCH_START, ''))
        ...
        case API_ACTIONS.FETCH_SUCCESS:
            const innerSuccess = camelCase(action.type.replace(API_ACTIONS.FETCH_SUCCESS, ''))
        ...
        case API_ACTIONS.FETCH_FAILURE:
            const innerFailure = camelCase(action.type.replace(API_ACTIONS.FETCH_FAILURE, ''))
